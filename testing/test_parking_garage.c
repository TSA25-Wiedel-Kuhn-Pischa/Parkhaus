...
//Unit-Tests für das Parkhaus:
  ...
  test_car3.arrival_time = 5;
  time_step = 8;
  assert(check_parking_time(&test_car3, time_step) == 1);             //Test bei erreichter maximaler Parkdauer

  //Überprüfung, ob die Funktion einen Fehlerwert zurückgibt
  assert(check_parking_time(&test_car3, time_step) == 1 || check_parking_time(&test_car3, time_step) == 0);
  
  test_car3.arrival_time = 7;
